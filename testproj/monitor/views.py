...
def calculateTime(df_log):
    ...
    return time_all
            ...
            time_last_close = row.time.split('.',1)[0]
            time_last_close1 = datetime.strptime(time_last_close, "%Y-%m-%dT%H:%M:%S")
            time_result[row.event_type]=time_last_close1
            time_result[row.username]='page_close'
            time_all.append(time_result)


df_time = calculateTime(df_log_test)
print(df_time)

def workDataFrame(df_log,df_csv):
    df_log = df_log[['username','time','event_type','page']]
    #df_csv = df_csv[['_c2','_c0','_c1','_c3','_c4','_c5','_c6','_c7','_c8']]
    #test_join = df_csv.join(df_log, df_csv._c2 == df_log.username, how='full')
    test_join = test_join.filter(test_join.username.isNotNull()).distinct()
    #test_join = test_join.filter((test_join._c2=="125507") & (test_join.course_id!=""))
    return test_join
def timeNextPage(test_start):
    if(test_start[['event_type']]=="page_close"):
        test_start = test_start.withColumn('next_time_page', F.lead(test_start['time']))
        return test_start
